...
def main(data_dir, file_name, class_num, class_name, out_dir):
    ...
    # 画像の読み込み
    img_src = cv2.imread(f'{data_dir}/images/{class_num}/{file_name}')
    trans_img = []
    ...
    tmp_path = f'{data_dir}/{out_dir}'
    if not os.path.exists(tmp_path):
        os.makedirs(tmp_path)

    tmp_path = f'{data_dir}/inflated_labels'
    if not os.path.exists(tmp_path):
        os.makedirs(tmp_path)
    ...
    for i, img in enumerate(trans_img):
        ...
        if i < flip_num:
            copyfile(
                f'{data_dir}/labels/{class_num}/{text_file_name}',
                f'{data_dir}/inflated_labels/{class_name}/{new_file_name}.txt')
        else:
            with open(f'{data_dir}/inflated_labels/{class_name}/{new_file_name}.txt', 'w') as new_bb_data:
                with open(f'{data_dir}/labels/{class_num}/{text_file_name}', 'r') as bb_data:
